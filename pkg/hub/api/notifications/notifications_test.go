...
func TestGetNotificationGroups(t *testing.T) {
	...
	t.Run("responds with default when get user by id fails ", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		dbClient := db.NewMockClient(ctrl)
		authUser := authContext.User{Email: "test@kobs.io"}
		dbClient.EXPECT().GetUserByID(gomock.Any(), authUser.Email).Return(nil, fmt.Errorf("could not get users"))
	...
	t.Run("when it gets user with notification groups", func(t *testing.T) {
		...
		dbClient := db.NewMockClient(ctrl)
		authUser := authContext.User{Email: "test@kobs.io"}
		dbClient.EXPECT().GetUserByID(gomock.Any(), authUser.Email).Return(&userv1.UserSpec{Notifications: userv1.Notifications{Groups: []userv1.NotificationsGroup{{Title: "Incidents", Plugin: dashboardv1.Plugin{Name: "opsgenie", Type: "opsgenie", Cluster: "cluster"}}}}}, nil)
	...
	t.Run("when user has no notification groups and get teams fails", func(t *testing.T) {
		...
		dbClient := db.NewMockClient(ctrl)
		authUser := authContext.User{Email: "test@kobs.io"}
		dbClient.EXPECT().GetUserByID(gomock.Any(), authUser.Email).Return(&userv1.UserSpec{ID: "test@kobs.io"}, nil)
