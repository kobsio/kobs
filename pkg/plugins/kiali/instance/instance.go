...
type Instance interface {
	...
	GetApplicationGraph(ctx context.Context, namespace, application string, duration int64, graphType, groupBy string, injectServiceNodes bool, appenders []string) (*Graph, error)
...
func (i *instance) extendGraph(graph *Graph) *Graph {
...
func (i *instance) GetGraph(ctx context.Context, duration int64, graphType, groupBy string, injectServiceNodes bool, appenders, namespaces []string) (*Graph, error) {
	...
	return i.extendGraph(graph), nil
}

func (i *instance) GetApplicationGraph(ctx context.Context, namespace, application string, duration int64, graphType, groupBy string, injectServiceNodes bool, appenders []string) (*Graph, error) {
	graph, err := doRequest[*Graph](ctx, i.client, fmt.Sprintf("%s/kiali/api/namespaces/%s/applications/%s/graph?duration=%ds&graphType=%s&injectServiceNodes=%t&groupBy=%s&appenders=%s", i.address, namespace, application, duration, graphType, injectServiceNodes, groupBy, strings.Join(appenders, ",")))
	if err != nil {
		return nil, err
	}
