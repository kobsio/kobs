...
import { IDataView, ILogsData, example } from '../utils/utils';
...
const LogsPanelQuery: FunctionComponent<{
  ...
  const dataView = instance.options?.dataViews?.find((dataView: IDataView) => dataView.name === query.dataView);

  const { isError, isLoading, error, data, refetch } = useQuery<ILogsData, APIError>(
    ['elasticsearch/logs', query.dataView, query.query, times.timeStart, times.timeEnd],
    async () => {
...
const ElasticsearchPanel: FunctionComponent<IPluginPanelProps<IOptions>> = ({
  title,
  description,
  options,
  instance,
  ...
  return (
    <PluginPanelError
      title={title}
      description={description}
      message="Invalid options for Elasticsearch plugin"
