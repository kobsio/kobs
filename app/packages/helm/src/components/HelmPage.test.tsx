...
describe('HelmPage', () => {
  ...
  it('should render info that clusters are required', async () => {
    ...
    expect(screen.getByText('(hub / helm)')).toBeInTheDocument();
  ...
  it('should render list of helm releases', async () => {
    ...
    await userEvent.click(screen.getByText(/Values/));

    await userEvent.click(screen.getByText(/Templates/));
    expect(await waitFor(() => screen.getByText('templates/NOTES.txt'))).toBeInTheDocument();
