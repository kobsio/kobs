import { IPluginPanelProps, PluginPanel, PluginPanelActionLinks, PluginPanelError, pluginBasePath } from '@kobsio/core';
...
/**
 * The `VeleroPanel` is the panel component for the Flux plugin, so that the plugin can be used within a dashboard.
 */
const VeleroPanel: FunctionComponent<IPluginPanelProps<IOptions>> = ({
  title,
  description,
  options,
  instance,
  ...
  if (
    ...
    (options.type === 'backups' || options.type === 'restores' || options.type === 'schedules')
  ) {
  ...
  return (
    <PluginPanelError
      title={title}
      description={description}
      message="Invalid options for Velero plugin"
      details="One of the required options is missing."
      example={example}
      documentation="https://kobs.io/main/plugins/velero"
    />
  );
};

export default VeleroPanel;
