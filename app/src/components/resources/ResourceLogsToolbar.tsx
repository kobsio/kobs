...
interface IResourceLogsToolbarProps {
  options: IOptions;
  setOptions: (options: IOptions) => void;
}

// ResourceLogsToolbar is the toolbar, to set some options for the Kubernetes logs request. The user can provide a
...
const ResourceLogsToolbar: React.FunctionComponent<IResourceLogsToolbarProps> = ({
  options,
  setOptions,
}: IResourceLogsToolbarProps) => {
  ...
  const [regex, setRegex] = useState<string>(options.regex);
  const [since, setSince] = useState<number>(options.since);
  const [showContainer, setShowContainer] = useState<boolean>(false);
  const [showModal, setShowModal] = useState<boolean>(false);

  // onEnter is used so that a user can trigger the search by pressing enter in the regex input field.
            ...
            <ToolbarGroup style={{ alignItems: 'flex-start', width: '100%' }}>
              <ToolbarItem style={{ width: '100%' }}>
                <TextInput
                  ...
                  placeholder="Regex"
                  value={regex}
                  onChange={(value: string): void => setRegex(value)}
                  onKeyDown={onEnter}
                />
              </ToolbarItem>
              <ToolbarItem>
...
export default ResourceLogsToolbar;
