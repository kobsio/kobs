...
interface IKialiDetailsEdgeMetricsGRPCProps {
  name: string;
  duration: number;
  edge: Edge.AsObject;
...
// KialiDetailsEdgeMetricsGRPC is used to get the the data for the grpc metrics (request traffic and request response
// time).
const KialiDetailsEdgeMetricsGRPC: React.FunctionComponent<IKialiDetailsEdgeMetricsGRPCProps> = ({
  name,
  duration,
  edge,
  sourceNode,
  targetNode,
}: IKialiDetailsEdgeMetricsGRPCProps) => {
  ...
  const fetchGraph = useCallback(async (): Promise<void> => {
    try {
      ...
      getMetricsRequest.setRequestprotocol('grpc');
  ...
  return (
    <React.Fragment>
      {data.traffic.length === 0 ? (
        ...
        <Card isCompact={true}>
          <CardTitle>GRPC Request Traffic</CardTitle>
          <CardBody>
      ...
      {data.responseTime.length === 0 ? (
        ...
        <Card isCompact={true}>
          <CardTitle>GRPC Request Response Time</CardTitle>
          <CardBody>
...
export default KialiDetailsEdgeMetricsGRPC;
