...
const Projects: React.FunctionComponent<IProjectsProps> = ({ instance, query }: IProjectsProps) => {
  ...
  const { isError, isLoading, error, data, refetch } = useQuery<IResponseProjects, Error>(
    ['sonarqube/projects', instance, query, page],
    async () => {
      try {
        const response = await fetch(
          `/api/plugins/sonarqube/projects?query=${encodeURIComponent(query)}&pageNumber=${page.page}&pageSize=${
            page.perPage
          }`,
          {
            headers: {
    ...
    <div>
      <Accordion asDefinitionList={false}>
        {data.components.map((project) => (
          <ProjectsItem key={project.key} instance={instance} project={project} />
        ))}
      </Accordion>
