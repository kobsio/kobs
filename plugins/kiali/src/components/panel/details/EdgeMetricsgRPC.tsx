...
interface IEdgeMetricsgRPCProps {
  name: string;
  times: IPluginTimes;
...
const EdgeMetricsgRPC: React.FunctionComponent<IEdgeMetricsgRPCProps> = ({
  name,
  times,
  sourceNode,
  targetNode,
}: IEdgeMetricsgRPCProps) => {
  const { isError, isLoading, error, data, refetch } = useQuery<IChart[], Error>(
    ['kiali/metrics/edge/http', name, times, sourceNode, targetNode],
    async () => {
      try {
...
export default EdgeMetricsgRPC;
