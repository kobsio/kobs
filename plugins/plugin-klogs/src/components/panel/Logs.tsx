...
const Logs: React.FunctionComponent<ILogsProps> = ({ instance, title, description, queries, times }: ILogsProps) => {
  ...
  const { isError, isFetching, isLoading, data, error, refetch } = useQuery<ILogsData, Error>(
    ['klogs/logs', instance, selectedQuery, times],
    async () => {
      try {
        ...
        const response = await fetch(
          `/api/plugins/klogs/logs?query=${encodeURIComponent(selectedQuery.query)}&order=${
            selectedQuery.order || ''
  ...
  return (
    <PluginPanel
      ...
      actions={<LogsActions instance={instance} queries={queries} times={times} isFetching={isFetching} />}
    >
      <div>
        {queries.length > 1 ? (
          <div>
