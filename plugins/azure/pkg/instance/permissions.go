...
func (i *Instance) CheckPermissions(r *http.Request, resource, resourceGroup string) error {
	...
	permissionsInterface, err := user.GetPluginPermissions(chi.URLParam(r, "name"))
	if err != nil {
		return err
	}

	permissions, ok := permissionsInterface.([]Permissions)
	if !ok {
		return fmt.Errorf("the permissions are invalid")
	}

	if hasAccess(resource, resourceGroup, strings.ToLower(r.Method), permissions) {
		return nil
	}
