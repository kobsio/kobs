...
const Incidents: React.FunctionComponent<IPluginNotificationsProps> = ({
  title,
  options,
  instance,
  times,
  ...
  const { data, refetch } = useQuery<IIncident[], Error>(['opsgenie/incidents', instance, options, times], async () => {
    try {
      const response = await fetch(
        `/api/plugins/opsgenie/incidents?query=${queryWithTime(options.query, times, options.interval)}`,
        {
          headers: {
  ...
  return (
    ...
    >
      <NotificationDrawerList isHidden={!isExpanded}>
        {data.map((incident) => (
          <Incident key={incident.id} instance={instance} incident={incident} refetch={refetch} />
        ))}
