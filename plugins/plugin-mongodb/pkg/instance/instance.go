...
func (i *instance) Find(ctx context.Context, collectionName string, query []byte, limit int64, sort string) ([]bson.D, error) {
	var bsonQuery any
	...
	cursor, err := i.mongoClient.Database(i.config.DatabaseName).Collection(collectionName).Find(ctx, bsonQuery, &mongoOptions.FindOptions{
		Limit: &limit,
		Skip:  nil,
		Sort:  bsonSort,
	})
...
func (i *instance) Count(ctx context.Context, collectionName string, query []byte) (int64, error) {
	var bsonQuery any
	err := bson.UnmarshalExtJSON(query, false, &bsonQuery)
	if err != nil {
		log.Error(ctx, "Could not unmarshall query", zap.Error(err))
	...
	return i.mongoClient.Database(i.config.DatabaseName).Collection(collectionName).CountDocuments(ctx, bsonQuery)
}

func (i *instance) FindOne(ctx context.Context, collectionName, filter string) (*bson.M, error) {
	var bsonQuery any
	err := bson.UnmarshalExtJSON([]byte(filter), false, &bsonQuery)
	if err != nil {
		log.Error(ctx, "Could not unmarshall query", zap.Error(err))
		return nil, err
	}

	res := i.mongoClient.Database(i.config.DatabaseName).Collection(collectionName).FindOne(ctx, bsonQuery)
	if res.Err() != nil {
