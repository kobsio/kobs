...
func (router *Router) getContainerGroups(w http.ResponseWriter, r *http.Request) {
	...
	for _, resourceGroup := range resourceGroups {
		...
		if err == nil {
			...
			log.Warn(r.Context(), "User is not authorized to get container groups", zap.String("resourceGroup", resourceGroup), zap.String("name", name), zap.String("user", user.Email), zap.String("method", r.Method), zap.Error(err))
		}
	}
